"""Defines macros to for tooling-related actions.
"""


def lint_sources(name, sources, compdb, dependencies=None, options=None, visibility=None):
    """Runs linting on given source files.

    This macro defines a test target that depends on intermediate lint rules. An
    intermediate lint rule is created for every item in `files`. Note that this
    target specifies the `manual` tag.
